...
class _TTLCache:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> Any:
        self._entries[str(key)] = _Entry(
            value=value,
            expires_at=time.time() + max(1, int(ttl or self.default_ttl)),
        )
        return value
    ...
    def stats(self, key: str) -> dict[str, Any]:
        ...
        if entry and entry.expires_at <= time.time():
            ...
            entry = None
        return {"hits": entry.hits if entry else 0, "present": entry is not None}


class ProviderPrefixCache(_TTLCache):
    """Track stable provider prompt prefixes for local observability."""
...
class SemanticCache(_TTLCache):
    ...
    def _key(intent: str, state: dict[str, Any] | None) -> str:
        fingerprint = hashlib.sha256(
            json.dumps(
                state or {}, ensure_ascii=False, sort_keys=True, separators=(",", ":")
            ).encode("utf-8")
        ).hexdigest()[:16]
