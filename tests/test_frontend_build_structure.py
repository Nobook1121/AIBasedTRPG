...
def test_tool_scripts_are_loaded_from_data_tools():
    ...
    assert 'src="js/tools/diceTool.js"' in html
    assert 'src="js/tools/toolManager.js"' in html
    assert 'src="data/tools/diceTool.js"' not in html
    assert 'src="data/tools/toolManager.js"' not in html
    assert 'src="tools/diceTool.js"' not in html


def test_tool_source_lives_in_data_tools_and_build_output_lives_in_js_tools():
    source_tools = PROJECT_ROOT / "data" / "tools"
    build_tools = PROJECT_ROOT / "js" / "tools"
    legacy_tools = PROJECT_ROOT / "frontend" / "src" / "tools"
    root_tools = PROJECT_ROOT / "tools"
    relocate_script = _read(PROJECT_ROOT / "scripts" / "relocate-tools.mjs")

    assert (source_tools / "diceTool.ts").exists()
    assert (source_tools / "toolManager.ts").exists()
    assert not (source_tools / "diceTool.js").exists()
    assert not (source_tools / "toolManager.js").exists()
    assert (build_tools / "diceTool.js").exists()
    assert (build_tools / "toolManager.js").exists()
    assert not (legacy_tools / "diceTool.ts").exists()
    assert not (legacy_tools / "toolManager.ts").exists()
    assert not root_tools.exists()
    assert "copyFile" in relocate_script
    assert 'path.join(root, "data", "tools")' in relocate_script
    assert 'path.join(root, "js", "tools")' in relocate_script
    assert "rename(" not in relocate_script
    assert "rm(sourceDir" not in relocate_script


def test_check_command_uses_backend_room_tool_instead_of_local_character_math():
    manager_source = _read(PROJECT_ROOT / "data" / "tools" / "toolManager.ts")
    chat_source = _read(CHAT_TS)

    assert "/api/rooms/${encodeURIComponent(roomId)}/tools/check" in manager_source
    assert "fetch(" in manager_source
    assert "async handleCheckCommand" in manager_source
    assert "await window.toolManager?.handleCommand" in chat_source
    assert "findCheckValue" not in manager_source
    assert "applyDifficulty" not in manager_source
    assert 'parseDiceCommand("1d100")' not in manager_source


def test_chat_renders_tool_messages_returned_by_ai_chat_api():
    chat_source = _read(CHAT_TS)

    assert "tool_messages?: ChatMessage[]" in chat_source
    assert "data.tool_messages || []" in chat_source
    assert "clearThinkingMessage(thinkingMessageId)" in chat_source
    assert "renderRoomMessage(toolMessage)" in chat_source
    assert "broadcastMessage(toolMessage)" in chat_source
    assert "renderRoomMessage(persisted)" in chat_source
...
def test_default_chinese_investigator_name_pool_is_large_enough_to_avoid_obvious_repetition():
    character_source = _read(CHARACTER_TS)
    china_entry = character_source[
        character_source.index("china: {"):
        character_source.index("japan: {")
    ]

    def names_from(field):
        marker = f"{field}: ["
        start = china_entry.index(marker) + len(marker)
        end = china_entry.index("]", start)
        return [
            item.strip().strip('"')
            for item in china_entry[start:end].split(",")
            if item.strip()
        ]

    family_names = names_from("family")
    male_names = names_from("male")
    female_names = names_from("female")
    neutral_names = names_from("neutral")

    assert len(family_names) >= 24
    assert len(male_names) >= 24
    assert len(female_names) >= 24
    assert len(neutral_names) >= 12
    assert len(family_names) * (len(male_names) + len(female_names) + len(neutral_names)) >= 1440


def test_unknown_gender_investigator_names_use_the_full_given_name_pool():
    character_source = _read(CHARACTER_TS)

    assert 'gender === "male" ? source.male : gender === "female" ? source.female : [...source.male, ...source.female, ...source.neutral]' in character_source
...
def test_icon_font_is_local_and_character_icon_buttons_have_accessible_fallbacks():
    head_source = _read(PROJECT_ROOT / "frontend" / "src" / "index" / "fragments" / "01-head-and-sidebar.html")
    fontawesome_css = PROJECT_ROOT / "data" / "assets" / "vendor" / "font-awesome" / "css" / "font-awesome.min.css"
    fontawesome_font = PROJECT_ROOT / "data" / "assets" / "vendor" / "font-awesome" / "fonts" / "fontawesome-webfont.woff2"

    assert "w3schools.cn/cdnjs/ajax/libs/font-awesome" not in head_source
    assert 'href="/assets/vendor/font-awesome/css/font-awesome.min.css"' in head_source
    assert fontawesome_css.exists()
    assert fontawesome_font.exists()
    assert "@font-face" in _read(fontawesome_css)
    assert "aria-label=" in _read(PROJECT_ROOT / "frontend" / "src" / "index" / "fragments" / "04-editor-modals.html")


def test_character_import_and_export_use_test_character_json_shape():
    index_source = _read(PROJECT_ROOT / "frontend" / "src" / "index" / "fragments" / "02-main-tabs.html")
    character_source = _read(CHARACTER_TS)

    assert 'id="importCharacter"' in index_source
    assert 'id="importCharacterFile"' in index_source
    assert 'accept=".json"' in index_source
    assert "interface TestCharacterJson" in character_source
    assert "function convertCardToTestCharacterJson" in character_source
    assert "function convertTestCharacterJsonToCardInput" in character_source
    assert 'byId("importCharacter")?.addEventListener("click"' in character_source
    assert 'byId<HTMLInputElement>("importCharacterFile")?.click()' in character_source
    assert 'byId<HTMLInputElement>("importCharacterFile")?.addEventListener("change", importCharacterFiles)' in character_source
    assert "FileReader" in character_source
    assert "saveImportedCharacterFile" in character_source
    assert "JSON.stringify(convertCardToTestCharacterJson(card), null, 2)" in character_source
