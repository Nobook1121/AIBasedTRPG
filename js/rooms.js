...
function updateRoomDetail(room) {
    ...
    if (roomCode)
        roomCode.textContent = room.room_code || '-';
}
function updateRoomStatusBar(room) {
    ...
    if (statusName)
        statusName.textContent = room.name;
    if (statusScenario)
        statusScenario.textContent = room.scenario_title ? `剧本：${room.scenario_title}` : '';
}
